...
export const OrderListItem = ({ order }: Props) => {
	return (
		<li className="bg-card">
			<div className="flex flex-col gap-2 border border-border bg-secondary/40 px-6 py-4 md:grid md:grid-cols-4 md:gap-8">
				<dl className="flex flex-col divide-y divide-border text-sm md:col-span-3 md:grid md:grid-cols-3 md:gap-6 md:divide-none lg:col-span-2">
					<div className="flex flex-row items-center justify-between py-4 md:flex-col md:items-start md:gap-y-1">
						<dt className="font-medium text-foreground">Número de pedido</dt>
						<dd className="text-muted-foreground">{order.number}</dd>
					</div>
					<div className="flex flex-row items-center justify-between py-4 md:flex-col md:items-start md:gap-y-1">
						<dt className="font-medium text-foreground">Fecha de pedido</dt>
						<dd className="text-muted-foreground">
							<time dateTime={order.created}>{formatDate(new Date(order.created))}</time>
						</dd>
					</div>
					<div className="flex flex-row items-center justify-between py-4 md:flex-col md:items-start md:gap-y-1">
						<dt className="font-medium text-foreground">Estado de pago</dt>
						<dd>
			...
			{order.lines.length > 0 && (
				<>
										...
										<tr key={product.id}>
											<td className="py-6 pr-6 md:w-[60%] lg:w-[70%]">
												<div className="flex flex-row items-center">
													{product.thumbnail && (
														<div className="mr-3 aspect-square h-16 w-16 shrink-0 overflow-hidden rounded-md border border-border bg-secondary md:mr-6 md:h-24 md:w-24">
															<Image
													...
													<div>
														...
														{item.variant.name !== item.variant.id && Boolean(item.variant.name) && (
															<p className="mt-1 text-muted-foreground">Opción: {item.variant.name}</p>
														)}
													</div>
												</div>
											</td>
					...
					<dl className="flex justify-between border-y border-border py-6 text-sm font-medium text-foreground md:border md:px-6">
